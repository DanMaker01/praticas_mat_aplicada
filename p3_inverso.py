...
def analisar_falta_dagua(rede_func, nome_rede="Rede", qtd_ensaios=100, p_min_limite=1.0, 
    ...
    - p_min_limite: pressão mínima permitida
    ...
    # print(f"  - Pressão mínima limite: {p_min_limite}")
    ...
    for i in range(qtd_ensaios):
        ...
        for j in range(qtd_redes_com_perturbacao):
            ...
            # Seleciona valores importantes (remove zeros antes de estatísticas)
            mask = np.ones_like(p, dtype=bool)
            mask[nos_pressao_atm] = False
            p_validas = p[mask]
            
            menor_pressao_na_rede = np.min(p_validas)
            media_pressao_na_rede = np.mean(p_validas)
            
            pressoes_minimas.append(menor_pressao_na_rede)
            pressoes_medias.append(media_pressao_na_rede)
            
            if menor_pressao_na_rede < p_min_limite:
                cont_redes_faltou_pressao += 1
        
        # Calcula estatísticas do ensaio
        prob_estouro = float(cont_redes_faltou_pressao) / float(qtd_redes_com_perturbacao)
        historico[i] = prob_estouro
        
        # print(f"Ensaio {i+1:3d}/{qtd_ensaios} | Pouca pressão em: {cont_redes_faltou_pressao:4d}/{qtd_redes_com_perturbacao:4d} entupimentos"
    ...
    # print(f"{'='*70}")
    print(f"Probabilidade de falta de pressão (p < {p_min_limite}):")
    
    print(f"  - qtd_ensaios:      {qtd_ensaios}. \tqtd_redes_com_perturbação: {qtd_redes_com_perturbacao}")
...
if __name__ == "__main__":
    print("P3_PROBLEMA_INVERSO")
    # Configurações
    ALFA = 10e2  # Fator de entupimento (aumenta resistência em 1000x)
    VETOR_CHANCE_ENTUPIMENTO = [0.2, 0.1, 0.06, 0.04, 0.03, 0.02, 0.01]  # Probabilidade de cada aresta entupir (r)
    QTD_ENSAIOS = 10
    P_MIN_LIMITE = 1.0
    PORCENTAGEM_DESEJADA = 0.05

    historia = {}
    for r in VETOR_CHANCE_ENTUPIMENTO:
        # Teste com rede simples
        nome_rede_1 = f"REDE MEDIA r={r}"
        historico_simples, media_simples, desvio_simples = analisar_falta_dagua(
            rede_func=rede_media,
            nome_rede=nome_rede_1,
            qtd_ensaios=QTD_ENSAIOS,
            p_min_limite=P_MIN_LIMITE,
            alfa=ALFA,
            chance_entupimento=r
        )

        valores = list(historico_simples.values())
        chance_falta_de_pressao_pior_ensaio = np.max(valores)

        if chance_falta_de_pressao_pior_ensaio < PORCENTAGEM_DESEJADA:
            print(f"com r={r}, a chance de ocorrer pressão inferior a {P_MIN_LIMITE} é menor que {100*PORCENTAGEM_DESEJADA}%\n")
    ...
    # print(historico_simples)
    
    # # Teste com rede média
    # nome_rede_2 = "REDE MÉDIA (8 nós, 12 arestas)"
    # historico_media, media_media, desvio_media = analisar_entupimentos(
    #     rede_func=rede_media,
    #     nome_rede=nome_rede_2,
    #     qtd_ensaios=QTD_ENSAIOS,
    #     p_max_limite=P_MAX_LIMITE,
    #     alfa=ALFA,
    #     chance_entupimento=CHANCE_ENTUPIMENTO
    # )
    # # print(historico_media)
    
    # # Comparação final
    # print(f"\n{'='*70}")
    # print(f"COMPARAÇÃO ENTRE REDES")
    # print(f"{'='*70}")
    # print(f"{nome_rede_1}:  {100*media_simples:.2f}% ± {100*desvio_simples:.2f}%")
    # print(f"{nome_rede_2}:    {100*media_media:.2f}% ± {100*desvio_media:.2f}%")
