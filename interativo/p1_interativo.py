...
def rede_simples():
    """Gera uma rede simples com 4 vértices e 4 arestas (rede_simples original)"""
    vertices = [0, 1, 2, 3]
    arestas = [(0, 1), (0, 2), (1, 2), (1, 3)]
    nos_pressao_atm = [3]  # nó 3 fixo (atmosférico)
    R = [2, 3, 1, 2]
    Q = [12, 0, 0, 0]  # vazão de entrada no nó 0
...
def adicionar_ruido(alfa, chance_mudanca=0.1):
    global R
...
# ============================
# INICIALIZAÇÃO DA REDE MÉDIA
# ============================
# Inicializa com a rede_media
vertices, arestas, R, Q, nos_pressao_atm = rede_media()

# Cria os nós com posições aleatórias
nodes = [{"pos": (random.randint(x_min, x_max), random.randint(y_min, y_max))} for _ in vertices]
edges = arestas
pressao_atm = nos_pressao_atm
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # teclas globais (sem seleção)
                ...
                if event.key == pygame.K_n:
                    ...
                    selected_edge = None
                    creating_edge = None
                # adicionar ruído global (tecla T)
                elif event.key == pygame.K_t:
                    alfa = 100.0
                    adicionar_ruido(alfa)
